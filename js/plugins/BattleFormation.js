...
(function() {
    ...
    Sprite_Actor.prototype.setActorHome = function(index) {
        ...
        var xhome = [610+ a * 40, 200 + b * 40];//bob
        var home = xhome;//this._actor._bfHome;//bob
        //var home = this._actor._bfHome;
        if(!$gameSwitches.value(111)){//中断スイッチ制御
        	this.setHome(home[0], home[1] + 80);//bob
        }else{
        	this.setHome(home[0], 0);//bob
        }
    ...
    Window_BFFormationList.prototype.initialize = function(rect) {// y bob
        if ($gameParty.battleFormation() === undefined) $gameParty.setBattleFormation(null);
        //this._oy = y;
        Window_Command.prototype.initialize.call(this,rect);//this, 0, y bob
        this.openness = 0;
    ...
    Window_BFFormationList.prototype.maxPageItems = function() {
        ...
        //var max = ((Graphics.boxHeight - hw) - this.standardPadding() * 2) / this.lineHeight();bob
        var max = ((Graphics.boxHeight - hw) - this._padding * 2) / this.lineHeight();
    ...
    Window_BFFormationList.prototype.refresh = function() {
        ...
        //var mh = this.lineHeight() * this.maxPageItems() + this.standardPadding() * 2;//bob
        var mh = this.lineHeight() * this.maxPageItems() + this._padding * 2;
        console.log("この問題");
        console.log(this);
        //this.height = Math.min(l * this.lineHeight() + this.standardPadding() * 2 + this.offsetHeight(),mh);
        this.height = Math.min(l * this.lineHeight() + this._padding * 2 + this.offsetHeight(),mh);
    ...
    Window_BFFormationList.prototype.drawItem = function(index) {
        //var rect = this.itemRectForText(index);//bob
        let rect = this.itemRect(index);
        rect.x += this._padding;//textPadding();
        rect.width -= this._padding*2;//textPadding() * 2;
        ...
        if (exps.length > 0) {
            ...
            console.log("ここですぜ");
            //this.drawGauge(rect.x + 172, rect.y + 8, rect.width - 168, rate, gc1,gc2);bob
    ...
    Window_BFFormationList.prototype.drawFormationLevel = function(rect, lv, max, item) {
        ...
        //color = this.textColor(color);//bob
        if (isNaN(symbol)) {
            if (symbol.length > 1) {
    ...
    Window_BFStatus.prototype.initialize = function(rect,x,y) {//x,y bob
        this._frameCount = 0;
        this._lockIndex = -1;
        var w = Graphics.boxWidth - x;
        var h = Graphics.boxHeight - y + 4;
        Window_Selectable.prototype.initialize.call(this,rect);//this, x, y, w, h bob
        console.log("かいへんしても２");
        this._actorSprites = [];
        var members = $gameParty.battleMembers();
        for (var i=0,max=members.length;i<max;i++) {
            var actor = members[i];
            //var bitmap = ImageManager.loadSvActor(actor._battlerName);//bob
            var bitmap = ImageManager.loadSvActor(actor._priorityBattlerName);//bob
            if(!actor._priorityBattlerName){bitmap = ImageManager.loadSvActor(actor._battlerName);}//bob            console.log(actor);
    ...
    //Window_BFParameter.prototype = Object.create(Window_Base.prototype);bob
    ...
    Window_BFParameter.prototype.initialize = function(rect) {
        Window_Selectable.prototype.initialize.call(this,rect);//0,0,144,50 bob
        this.openness = 0;
    };
    ...
    Window_BFParameter.prototype.refresh = function() {
        if (showParamWindow && this._commandWindow && this._statusWindow) {
            ...
            ary = ary.concat(ary2);
            var size = 18;
            //this.height = (ary.length * (size + 2)) + this.lineHeight() + this.standardPadding() * 2 + 8;//bob
            this.height = (ary.length * (size + 2)) + this.lineHeight() + this._padding * 2 + 8;
            ...
            //this.height = (n * (size + 2)) + this.lineHeight() + this.standardPadding() * 2 + 8;bob
            this.height = (n * (size + 2)) + this.lineHeight() + this._padding * 2 + 8;
        }
    };
    ...
    Scene_Menu.prototype.create = function() {
        ...
        //this.createFormationInfoWindow(); bob
    };
    ...
    Scene_BattleFormation.prototype.createCommandWindow = function() {
        let y = 0;//this._helpWindow.y + this._helpWindow.height;bob

        let x = 0;// + this._helpWindow.width;//bob
        let wx = this._helpWindow.width;//bob
        let wy = this._helpWindow.height;//bob
        this._commandWindow = new Window_BFFormationList(new Rectangle(x,y,wx,wy));//y bob
    ...
    Scene_BattleFormation.prototype.createStatusWindow = function() {
        var x = 0;//this._commandWindow.x + this._commandWindow.width;
        var y = 0;//this._helpWindow.y + this._helpWindow.height;

        let wx = this._commandWindow.width;//bob
        let wy = this._helpWindow.height;//bob

        this._statusWindow = new Window_BFStatus(new Rectangle(x,y,wx,wy));//x,y bob
    ...
    Scene_BattleFormation.prototype.createParameterWindow = function() {
        this._paramWindow = new Window_BFParameter(new Rectangle(0,0,144,50));//bob
    ...
    Scene_BattleFormation.prototype.setFormation = function() {
    	AudioManager.playSe({"name":"YY-daon","volume":90,"pitch":100,"pan":0});//ボブbob
